...
pub const VTable = struct {
    ...
    loadSkin: *const fn(ptr: *anyopaque, skin: *Skin, renderer: *Renderer) anyerror!void,

    render: *const fn(ptr: *anyopaque, renderer: *Renderer, timestamp: u64) anyerror!void
};
...
pub fn loadSkin(self: *Replayer, skin: *Skin, renderer: *Renderer) !void {
    try self.vtable.loadSkin(self.unmanaged, skin, renderer);
}

// Render a frame.
pub fn render(self: *Replayer, renderer: *Renderer, timestamp: u64) !void {
    try self.vtable.render(self.unmanaged, renderer, timestamp);
}
